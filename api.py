...
@app.post("/cars", tags=["Cars"], status_code=status.HTTP_201_CREATED, response_model=CreateCarResponse)
...
def add_person(input: PersonRequest, session: Session = Depends(get_session)):
    res = add_person(session, **input.__dict__)
    return res


@app.delete("/car/{id}", tags=["Person"], status_code=status.HTTP_204_NO_CONTENT)
def delete_car(id: uuid.UUID, session: Session = Depends(get_session)):
    return delete_car_object(session, id)
