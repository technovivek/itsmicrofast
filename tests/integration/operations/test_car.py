...
def test_add_car_object(sql_mock, test_database, car_object):
    ...
    assert "id" in add_car_object(
        make=car_object.make, model=car_object.model, price=car_object.price
    )
...
def test_added_car(test_database, car_object):
    ...
    # test_database.add(car_object)
    # print("res---->", [r for r in test_database.query(car_object)
    # if r.model == car_object.model])
